...
def selection(
    ...
    for candidate in candidates:
        load_layers(model, grouped_layer_names, candidate, sparse_weights_path)
...
def main():
    ...
    configuration_name = f"evo-{args.fitness_fn}-configuration-{args.target_level}.txt"
    with open(os.path.join(args.sparse_weights_path, configuration_name), "w") as f:
        for i in range(num_groups):
            f.write(
